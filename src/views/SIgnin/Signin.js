...
const Signin = () => {
    ...
    const [KeepMesSigned, setKeepMesSigned] = useState(false)



    const handleChange = () => setKeepMesSigned(!KeepMesSigned)
    ...
    return (
        <section className='min-h-screen w-full grid md:grid-cols-2 grid-cols-1 items-center'>
            <div className='flex justify-center items-center flex-col '>
                <div>

                    <h1 className='capitalize md:text-4xl text-2xl font-bold font-primary text-center md:text-left'>sign in</h1>
                    <div className="mt-2 flex justify-center md:justify-start">
                        <h3 className='capitalize md:text-base text-sm font-bold font-primary  '>new user ?</h3>
                        <Link to='' className='md:text-base text-sm font-semibold font-secondary ml-2'>Create an account</Link>
                    </div>

                    {/* sign in form */}
                    <div className='mt-4'>
                        <InputField className={'p-2 outline-primary outline outline-2 md:w-72 w-60 text-sm'} type={'text'} name={'username'} placeholder={'Username or email'} onChange={setdata} value={data.username} />
                    </div>
                    <div className='mt-4'>
                        <InputField className={'p-2 outline-primary outline outline-2 md:w-72 w-60 text-sm'} type={'password'} name={'password'} placeholder={'Password'} onChange={setdata} value={data.password} />
                    </div>
                    {/* sign in form */}

                    {/* check box */}
                    <div className='mt-4 relative flex items-center'>
                        <InputField className={'p-2 outline-primary outline outline-2 z-10 absolute'} type={'checkbox'} name={'keepMeSigned'} onChange={handleChange} checked={KeepMesSigned} />

                        <div className={KeepMesSigned ? 'absolute inset-0 z-0' : 'hidden'}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="15.533" height="15.532" viewBox="0 0 15.533 15.532">
                                <path id="Check_DISPLAY_ELEMENTS:Label_STATE:ACTIVE_" data-name="Check [DISPLAY_ELEMENTS:Label][STATE:ACTIVE]" d="M2279.346,1602.008v6.359h11.607" transform="translate(-2743.118 488.572) rotate(-45)" fill="none" stroke="#000" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
                            </svg>
                        </div>

                        <div className='ml-7'>
                            <p className='sm:text-sm text-xs font-semibold font-primary'>Keep me signed</p>
                        </div>
                    </div>
                    {/* check box */}
                    ...
                    {/* submit button */}
                    <div className='mt-5'>
                        <button className='md:w-72 w-60 bg-primary text-white sm:text-sm text-xs font-semibold capitalize p-2' type='button' onClick={handleSubmit}>
                            sign in
                        </button>
                    </div>
                    {/* submit button */}
                    ...
                    <div className='relative mt-10 md:w-72 w-60'>
                        <div className="border border-1 border-primary"></div>
                        <div className="absolute md:w-72 w-60 flex justify-center inset-0">
                            <p className='font-primary w-32 bg-white h-5  text-center -mt-2 font-bold sm:text-sm text-xs'>
                                or sign in with
                            </p>
                        </div>
                    </div>
...
export const GirlSvg = <svg xmlns="http://www.w3.org/2000/svg" width="277.763" height="412" viewBox="0 0 277.763 412">
    <g id="Illustration" transform="translate(24.158)">
        ...
        <path id="Path_FILL:STYLE1_foreground_" data-name="Path [FILL:STYLE1/foreground]" d="M419.742,300.409,438.175,337.9,470.7,404.073l33.538-16.116L467.219,301.28l-.1-10.911-.331-33.952Z" transform="translate(-280.468 -34.423)" fill="#3d3d3d" />
